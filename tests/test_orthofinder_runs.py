...
@pytest.mark.order(1)
def test_orthofinder_dendroblast_run(projects, gene_tree_method, capfd):
...
@pytest.mark.order(2)
def test_orthofinder_mafft_run(projects, gene_tree_method, capfd):
...
@pytest.mark.order(3)
def test_orthofinder_assign_run(projects, assign, gene_tree_method, capfd):
...
@pytest.mark.order(4)
def test_orthofinder_assign_with_species_tree_run(projects, assign, species_tree_assign, gene_tree_method, capfd):
    results_dir = os.path.join(projects, "OrthoFinder")
    projects_results = helper._latest_output_dir(results_dir, fileno=-2)
    args = ["--core", projects_results, "--assign",  assign, "-nk", 2, "-s", species_tree_assign]
...
@pytest.mark.order(5)
def test_orthofinder_famsa_species_tree_run(projects, gene_tree_method, species_tree, capfd):
    args = ["-f", projects, "-s", species_tree]
...
@pytest.mark.order(6)
def test_orthofinder_prepare_blast_run(projects, gene_tree_method, capfd):
    args = ["-f", projects, "-op"]
...
# # @pytest.mark.order(2)
# # def test_orthofinder_start_from_blast_run(projects, gene_tree_method, capfd):
...
# #         args += ["-T", gene_tree_method]
    ...
    #     assert e.code == 0  # treat clean exit as success

    # out, err = capfd.readouterr()
    # assert "ERROR" not in err

@pytest.mark.order(7)
def test_orthofinder_old_version_run(projects, gene_tree_method, capfd):
...
@pytest.mark.order(8)
def test_orthofinder_no_file_fix_run(projects, gene_tree_method, capfd):
...
@pytest.mark.order(9)
def test_orthofinder_blast_run(projects, gene_tree_method, capfd):
...
def test_orthofinder_dna_run(dna_projects, capfd):
...
def test_orthofinder_oneway_search_run(projects, gene_tree_method, capfd):
...
def test_orthofinder_iqtree_run(projects, capfd):
...
def test_orthofinder_fasttree_fastest_run(projects, capfd):
...
def test_orthofinder_raxml_run(projects, capfd):
...
def test_orthofinder_famsa_run(projects, gene_tree_method, capfd):
    args = ["-f", projects]
