...
@pytest.mark.order(1)
def test_orthofinder_direct_runs(direct_case, projects, dna_projects, species_tree, user_ofconfig, capfd):
...
@pytest.mark.order(2)
def test_orthofinder_stopafter_runs(stopafter_case, projects, dna_projects, species_tree, user_ofconfig, capfd):
    name, argstr = stopafter_case
    helper.run_orthofinder_core_case(name, argstr, projects, dna_projects, species_tree, user_ofconfig, capfd)

@pytest.mark.order(3)
def test_orthofinder_restart_runs(restart_case, projects, assign, species_tree_assign, user_ofconfig, capfd):
