            ...
            for h, hog_rows in d.items():
                ...
                for r in hog_rows:
                    ...
                    if h == "N0.ids":
                        ...
                        hog_tree_file = os.path.join(hog_tree_dir, hog_name_reformat + ".nwk")
                        hog_tree.write(outfile=hog_tree_file, format=5)                        
    ...
    try:
        ...
        while True:
            try:
                ...
                results = tree_analyser.AnalyseTree(iog)
                if results is None:
                    # indicates a failed tree parse or missing file
                    results_queue.put(False)  
                    continue

                # results => (nOrtho, olog_lines, olog_sus_lines)
                nOrtho, olog_lines, olog_sus_lines = results
                nOrthologues_SpPair += nOrtho
                nCache += nOrtho
                ...
                if fewer_open_files:
                    ...
                    for i, j in zip(I, J):
                        ...
                        WriteOlogLinesToFile(
                            ...
                            olog_lines_tot[i][j],
                            tree_analyser.lock_ologs[k_lock]  # consistent lock usage
                        )
                        olog_lines_tot[i][j] = ""
...
# def Worker_RunOrthologsMethod(tree_analyser, 
#                               nspecies, 
#                               iog, 
#                               results_queue, 
#                               fewer_open_files, 
...
#     task_size = args_queue.qsize()
#     progressbar, task = util.get_progressbar(task_size)
#     update_cycle = 1
...
#     runningProcesses = []
#     # nOrthologues_SpPair = util.nOrtho_sp(nspecies)
#     while not args_queue.empty() or runningProcesses:
#         if not args_queue.empty() and len(runningProcesses) < nProcesses:
#             iog = args_queue.get()
#             proc = mp.Process(
...
#         nOrthologues_SpPair = OrthoQueue(runningProcesses, nOrthologues_SpPair, results_queue, progressbar, task, update_cycle)

#     progressbar.stop()
#     return nOrthologues_SpPair

# def OrthoQueue(runningProcesses, nOrthologues_SpPair, results_queue, progressbar, task, update_cycle):

#     while runningProcesses:
#         for i, proc in enumerate(runningProcesses[:]):
