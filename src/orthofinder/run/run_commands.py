...
def RunSearch(
    ...
    """
    name_to_print = (
        "BLAST" if options.search_program == "blast" else options.search_program
    )
    ...
    if options.search_program == "blast":
        for f in glob.glob(
            files.FileHandler.GetWorkingDirectory1_Read()[0] + "BlastDBSpecies*"
        ):
            os.remove(f)
            ...
            if options.search_program == "blast":
                command = " ".join(
                    [
                        ...
                        fn_fasta,
                        "-out",
                        files.FileHandler.GetSpeciesDatabaseN(iSp),
                    ]
                )
                util.PrintTime(
                    "Creating Blast database %d of %d" % (iSp + 1, len(iSpeciesToDo))
                )
...
def CreateSearchDatabases_accelerate(speciesInfoObj, options, prog_caller):
    ...
    for i, iSp in enumerate(iSpeciesNew):
        ...
        if options.search_program == "blast":
            ...
            util.PrintTime(
                "Creating Blast database %d of %d" % (iSp + 1, len(iSpeciesNew))
            )
            RunBlastDBCommand(command)
        else:
            command = prog_caller.GetSearchMethodCommand_DB(
...
def RunSearch_accelerate(
    options, speciessInfoObj, fn_diamond_db, prog_caller, q_one_query=False
):
    name_to_print = (
        "BLAST" if options.search_program == "blast" else options.search_program
    )
